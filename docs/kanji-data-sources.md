...
- 배포된 카탈로그 API 기준 노출 데이터 consistency 리포트 생성
  ...
  - 장시간 진단 시 `--concurrency 12`, `--request-timeout-ms 60000`처럼 호출 동시성과 timeout을 조정할 수 있다.
...
`kanji_characters.metadata.specialReadings`는 일반 읽기 목록만으로 학습자가 예측하기 어려운 단어를 보조로 보여주기 위한 필드다.

유지 기준:

- 숙자훈 또는 관용적으로 굳어진 읽기처럼 단일 한자의 일반 음독/훈독 조합만으로 설명하기 어려운 단어
- 예: `今日(きょう)`, `今年(ことし)`, `明日(あした)`, `部屋(へや)`, `風邪(かぜ)`, `景色(けしき)`, `果物(くだもの)`, `文字(もじ)`, `時計(とけい)`, `枇杷(びわ)`, `桔梗(ききょう)`

제외 기준:

- 정상 음독/훈독 그대로 읽는 단어
- 연탁으로 설명 가능한 단어. 예: `こと → ごと`, `かみ → がみ`, `は → ば`
- 촉음화/반탁음화로 설명 가능한 단어. 예: `こく → こっ`, `しつ → しっ`, `ひき → ぴき`
- 해당 단어 안 모든 한자에 무조건 붙이지 않는다. 실제로 특수하게 읽히는 한자에만 붙인다.
- 예: `神主(かんぬし)`에서 `主`는 훈독 `ぬし` 그대로이므로 `主`의 특수 읽기로 보지 않는다.
- 예: `物事(ものごと)`, `仕事(しごと)`에서 `事`는 `こと → ごと` 연탁으로 설명 가능하므로 `事`의 특수 읽기로 보지 않는다.
...
`kanji_characters.metadata.words`는 예문과 별개로 학습자가 같이 볼 만한 대표 단어를 보여주기 위한 필드다.

데이터 생성 기준:

- 소스: `JMdict_e.gz` from EDRDG JMdict.
- 생성 스크립트: [scripts/build-kanji-words-from-jmdict.mjs](/Users/kangmihye/Desktop/study/seodang/scripts/build-kanji-words-from-jmdict.mjs:1)
- 실행: `npm run kanji:build:words`
- 결과 파일: `data/generated/kanji-words.generated.json`
- 업서트: `npm run supabase:upsert:enrichment -- --input data/generated/kanji-words.generated.json`

품질 기준:

- 예문에 이미 포함된 단어는 제외한다.
- `metadata.specialReadings`에 들어간 단어는 일반 단어 섹션에서 제외한다.
- `JMdict` 우선순위 태그가 있는 일반 표기만 사용한다.
- 숫자 기호, 반복 기호, 비표준/불규칙 표기는 제외한다.
- 한자 2-3자 복합어를 우선하고, 한 글자당 최대 3개만 저장한다.
- `meaningKo`는 `JMdict` 영어 gloss를 기반으로 기계 번역한 한국어 뜻을 채운다.
- `meaningJa`는 일본어 화면에서 뜻을 노출하지 않기 때문에 `null`로 둔다.
- 한국어 뜻은 앱에 먼저 반영하되, 어색한 기계 번역은 이후 품질 검수 배치에서 보정한다.
...
```json
{
  ...
  "words": [
    { "word": "日程", "reading": "にってい", "meaningKo": null, "meaningJa": null },
    { "word": "日常", "reading": "にちじょう", "meaningKo": "매일", "meaningJa": null }
  ]
}
```
