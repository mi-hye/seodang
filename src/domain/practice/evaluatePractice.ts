...
export function evaluatePractice({
  ...
  if (!template || template.length === 0) {
    return {
      ...
      drawnStrokes,
      expectedStrokes: 0,
      summary: "이 한자는 아직 기준 획 데이터가 준비되지 않았습니다.",
      feedback: ["샘플 stroke 템플릿이 없는 한자입니다."],
    };
  }
  ...
  for (let index = 0; index < matchedCount; index += 1) {
    ...
    if (!normalized) {
      feedback.push(`${index + 1}번째 획이 너무 짧게 입력되었습니다.`);
      continue;
    }
  ...
  if (countGap > 0) {
    if (drawnStrokes < expectedStrokes) {
      feedback.unshift(`획 수가 부족합니다. ${expectedStrokes}획 중 ${drawnStrokes}획을 썼습니다.`);
    } else {
      feedback.unshift(`획 수가 많습니다. ${expectedStrokes}획보다 많이 그렸습니다.`);
    }
  }
  ...
  if (feedback.length === 0) {
    feedback.push("획 수, 방향, 시작 위치가 기준과 전반적으로 잘 맞았습니다.");
  }
