...
async function fetchCharacterByLiteral(literal: string) {
  const rows = await supabaseFetchJson<KanjiCharacterRow[]>(
    `/rest/v1/kanji_characters?literal=eq.${encodeURIComponent(literal)}&select=id,literal,source,license,view_box_width,view_box_height&limit=1`,
    "Failed to fetch kanji character",
  );
...
async function fetchStrokesByCharacterId(characterId: string) {
  return supabaseFetchJson<KanjiStrokeRow[]>(
    `/rest/v1/kanji_strokes?character_id=eq.${encodeURIComponent(characterId)}&select=id,stroke_order,stroke_type,raw_type,direction,path,start_x,start_y,end_x,end_y,note&order=stroke_order.asc`,
    "Failed to fetch kanji strokes",
  );
}
